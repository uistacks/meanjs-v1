...
exports.getSubTypeServiceServices = function (req, res) {
    console.log(req.query.param1);
  // convert mongoose document to JSON
  
    Service.find({servicesubtype:req.query.param1}).exec(function (err, services) {
     if (err) {
      return res.status(422).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.update = function (req, res) {
  ...
  service.area = req.body.area;
  service.servicetype = req.body.servicetype;
  service.servicesubtype = req.body.servicesubtype;
